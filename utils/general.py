...
#######################################################################
def compute_custom_loss(p, targets, model):  # predictions, targets, model
    device = targets.device
    lcls, lbox, lobj = torch.zeros(1, device=device), torch.zeros(1, device=device), torch.zeros(1, device=device)
    tcls, tbox, indices, anchors, regression_xy = build_custom_targets(p, targets, model)  # targets
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if n:
            ...
            lregression = torch.zeros(1, device=device)
            lregression += mse_loss(pxy, regression_xy)
    ...
    lcls *= h['cls'] * s
    lregression *= h['giou'] * s
    bs = tobj.shape[0]  # batch size

    #loss = lregression + lobj
    loss = lbox + lobj
    return loss * bs, torch.cat((lregression, lobj, lcls, loss)).detach()

def build_custom_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        # Custom regression features
        regression_xy = gxy - gij

    return tcls, tbox, indices, anch, regression_xy
#######################################################################
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if gt or (conf[j] > 0.2 and conf[j] == max(conf)):  # 0.2 conf thresh and only plot the bb with highest conf
